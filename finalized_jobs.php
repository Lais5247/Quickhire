...
$query = "SELECT * FROM jobs 
          WHERE assigned_maid_id = :maid_id 
...
$total_stmt = $pdo->prepare("SELECT COUNT(*) FROM jobs WHERE assigned_maid_id = ?");
$total_stmt->execute([$maid_id]);
$total_jobs = $total_stmt->fetchColumn();

$assigned_stmt = $pdo->prepare("SELECT COUNT(*) FROM jobs WHERE assigned_maid_id = ? AND status = 'assigned'");
$assigned_stmt->execute([$maid_id]);
$assigned_jobs = $assigned_stmt->fetchColumn();

$completed_stmt = $pdo->prepare("SELECT COUNT(*) FROM jobs WHERE assigned_maid_id = ? AND status = 'completed'");
$completed_stmt->execute([$maid_id]);
...
<body>
    ...
    <div class="container">
        <h1><i class="fas fa-check-circle"></i> My Finalized Jobs</h1>
